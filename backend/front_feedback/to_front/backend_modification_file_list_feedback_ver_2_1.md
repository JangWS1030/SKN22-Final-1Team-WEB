...
현재 backend 코드 기준으로 다시 정리하면 아래처럼 보는 것이 가장 정확합니다.
...
즉 위 항목들은 “신규 개발 필요”보다는 “최신 구현 기준 확인 완료”에 가깝습니다.

### B. 실제 연동 시 충돌 가능성이 큰 항목

1. 공통 error envelope
- 현재 backend는 공통 `{ ok, error, message }` envelope를 사용하지 않습니다.
- validation error는 DRF 기본 필드별 에러, auth error는 `detail`입니다.

2. refresh token
- 현재 backend는 refresh token 흐름이 없습니다.
- `Authorization: Bearer {access_token}` 기준으로만 구현해 주셔야 합니다.
...
### C. 문서 수정 시 가장 중요한 반영 포인트

- “공통 error envelope 적용 필요” 문구는 현재 backend 기준과 다릅니다.
- “토큰 갱신 로직 강화 필요” 문구도 현재 backend 기준과 다릅니다.
- 최신 이미지 정책(`restricted_internal_store`)을 반드시 반영해 주세요.
- 이미 구현된 항목은 “추가 개발 필요”보다 “최신 응답 확인 필요”로 바꾸는 편이 정확합니다.

### D. 현재 backend 기준 구현 요약

- 관리자 화면은 mock -> API 전환이 가능한 수준까지 구조가 맞춰져 있습니다.
- 고객 화면은 route / auth / recommendation 중심 contract가 정리돼 있습니다.
- 이미지 미동의 시에는 내부 저장은 가능하지만 사용자/admin 모두 이미지 비노출입니다.
- `confirm / cancel`은 상태 동기화와 중복 호출 안전성까지 반영돼 있습니다.

### E. 프론트에 먼저 확인받고 싶은 4가지
